"""
src/train_dfine_small_lao_plate.py

Fine-tunes D-FINE Small (MIT License) for Lao Plate Detection.
Classes: plate (1 class / mapped from dataset).
Saves weights to weights/plate_detector_lao_dfine_small.pt and .onnx.
"""
...
TARGET_WEIGHTS_PATH = OUTPUT_WEIGHTS_DIR / "plate_detector_lao_dfine_small.pt"
TARGET_ONNX_PATH    = OUTPUT_WEIGHTS_DIR / "plate_detector_lao_dfine_small.onnx"
RUNS_DIR            = PROJECT_ROOT / "runs" / "train_dfine_small" / "lao_plate"
...
def train_dfine_small_lao_plate(epochs=30, batch=8, imgsz=640, patience=12, device="mps", export_onnx=True):
    print("=" * 70)
    print("🚀 D-FINE Small Training — Lao Plate Detection")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train D-FINE Small for Lao Plate Detection")
