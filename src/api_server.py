...
class LibreYOLOWrapper:
    """Wraps a LibreYOLO (D-FINE / RT-DETRv2 / PicoDet) model so it is callable identically to Ultralytics YOLO/RTDETR."""

    def __init__(self, model, names: dict | list | None = None, device: str = "cpu", default_imgsz: int | None = None):
        self.model = model
        self.device = str(device) if device else "cpu"
        # PREPROCESS FIX: PicoDet fine-tunes run at imgsz=416 (ONNX 416x416), but the
        # wrapper previously let predict() default to 640 -> scale mismatch vs training.
        # The pipeline now passes the per-model spec imgsz (see preprocess_registry).
    ...
    def __call__(self, img, conf: float = 0.25, imgsz: int | None = None, verbose: bool = False, device=None):
        ...
        if effective_imgsz is not None:
            kwargs["imgsz"] = int(effective_imgsz)
...
class RFDETRWrapper:
    ...
    def __call__(self, img, conf=0.25, imgsz=None, verbose=False, device=None):
        """RF-DETR inference with TRAINING-FAITHFUL preprocessing.

        rfdetr's predict() already does the exact training preprocessing:
        pure STRETCH to the variant's native square resolution (Base 560,
        Small 512, Nano 384 — ONNX-verified) + ImageNet normalization, and it
        returns boxes in the pixel grid of the image we pass in (no rescale
        needed). We therefore:
          - NEVER letterbox and NEVER pre-stretch ourselves,
          - keep the frame's aspect ratio and only cap the long side at 1600 px
            (a pure latency guard for HD/4K camera frames; detections keep
            sub-560 accuracy since the model stretches to native res anyway),
          - ignore `imgsz` for resolution: RF-DETR has a fixed native input
            size, unlike YOLO (the pipeline passes 640 for the RT-DETR family;
            forcing 640 here previously degraded quality for zero benefit).
        """
        import cv2
        import numpy as np
        ...
        if isinstance(img, np.ndarray):
            h, w = img.shape[:2]
            max_side = 1600  # latency guard only — geometry is handled by predict()
            if max(h, w) > max_side:
...
class LPRPipelineService:
    ...
    def __init__(self):
        ...
        # --- Per-model preprocessing specs (single source of truth) ---
    ...
    def _load_libreyolo_model(self, model_path: Path, class_names: list[str] | dict[int, str] | None = None, default_imgsz: int | None = None):
        ...
        return LibreYOLOWrapper(model, names=class_names, device=self.device, default_imgsz=default_imgsz)
    ...
    def process_image(
        ...
        # PREPROCESS FIX: use each model's TRAINED tensor size (PicoDet 416,
        # D-FINE 640, RF-DETR 560) instead of a blanket 640 — RFDETRWrapper
        # ignores imgsz entirely (native resolution), LibreYOLO needs the
        # exact train/export size. Ultralytics YOLO letterboxes adaptively,
        # so its old 1280-for-HD behavior is preserved.
        ...
        if isinstance(self.model_plate, LibreYOLOWrapper) and self.m1_imgsz_native is not None:
            m1_imgsz = self.m1_imgsz_native
        elif is_rtdetr_m1:
            m1_imgsz = 640
        else:
        ...
        if isinstance(self.model_plate, LibreYOLOWrapper) and self.m1_imgsz_native is not None:
            sens_imgsz = self.m1_imgsz_native
        else:
        ...
        if len(res1.boxes) == 0:
            ...
            if 1.2 <= aspect_ratio <= 4.5 and w_orig <= 1200 and h_orig <= 600:
                ...
                try:
                    # Same train-geometry upscale as the main Model 2 path.
                    m2_check_input, _ = upscale_to_train_geometry(test_resized, self.m2_spec)
                    res2_check = self.model_comp(m2_check_input, conf=0.25, verbose=False)[0]
        ...
        if country_name == "Thai":
            ...
            # PREPROCESS FIX: M2 was fine-tuned on ~2:1 rectified-plate crops at
            # 640x640 stretch. Feeding the 320x160 rectified plate made objects
            # ~3.5x smaller in tensor space than in training -> recall loss.
            # Upscale to the train pixel geometry (1280x640) before inference.
            m2_input, m2_scale = upscale_to_train_geometry(rectified_plate, self.m2_spec)
            try:
                res2 = self.model_comp(m2_input, conf=conf_m2, verbose=False, device=self.device)[0]
            except Exception:
                res2 = self.model_comp(m2_input, conf=conf_m2, verbose=False)[0]
            ...
            if len(res2.boxes) > 0:
                for c_box in res2.boxes:
                    ...
                    # POSTPROCESS FIX: map boxes from the upscaled grid back to
                    # rectified-plate pixels before clamping/cropping.
                    bx1, by1, bx2, by2 = [int(v) for v in divide_boxes_by_scale([bx1, by1, bx2, by2], m2_scale)[0]]
                    bx1, by1 = max(0, bx1), max(0, by1)
            ...
            # Same PREPROCESS FIX as the Thai branch (train geometry 1280x640).
            m2_input, m2_scale = upscale_to_train_geometry(flipped_plate, self.m2_spec)
            try:
                res2 = self.model_comp(m2_input, conf=conf_m2, verbose=False, device=self.device)[0]
            except Exception:
                res2 = self.model_comp(m2_input, conf=conf_m2, verbose=False)[0]
            ...
            if len(res2.boxes) > 0:
                for c_box in res2.boxes:
                    ...
                    c_conf = float(c_box.conf[0])
                    bx1, by1, bx2, by2 = c_box.xyxy[0].cpu().numpy().astype(int)
                    # POSTPROCESS FIX: undo the upscale before the flip-back math.
                    bx1, by1, bx2, by2 = [int(v) for v in divide_boxes_by_scale([bx1, by1, bx2, by2], m2_scale)[0]]
                    bx1, by1 = max(0, bx1), max(0, by1)
                    bx2, by2 = min(rw, bx2), min(rh, by2)
        ...
        if country_name == "Thai":
            ...
            if self.char_box_model is not None and self.char_classifier is not None and char_crop is not None:
                ...
                # PREPROCESS FIX: CLAHE removed. The char-box detector was trained on
                # raw crops (no CLAHE/unsharp at train time), so enhancing at inference
                # created a domain mismatch (duplicate boxes on complex Thai chars such
                # as ณ / ฌ). The model now sees the exact domain it was trained on.
                det_char_input = char_crop
                # Upscale to the train pixel geometry (~3.7:1 crops at a 560 tensor);
                # the per-axis scale is undone right after detection below.
                ...
                for b in box_res.boxes:
                    bx1, by1, bx2, by2 = [int(v) for v in divide_boxes_by_scale(b.xyxy[0].cpu().numpy(), m3a_scale)[0]]
            ...
            if self.char_box_model is not None and self.char_classifier_lao is not None and char_crop is not None and char_crop.size > 0:
                ...
                try:
                    ...
                    crop_w = char_crop.shape[1]
                    for b in box_res.boxes:
                        bx1, by1, bx2, by2 = [int(v) for v in divide_boxes_by_scale(b.xyxy[0].cpu().numpy(), lao_m3a_scale)[0]]
                        bconf = float(b.conf[0])
                        bw = bx2 - bx1
                        bh = by2 - by1
                        if bw < 8 or bh < 10:
