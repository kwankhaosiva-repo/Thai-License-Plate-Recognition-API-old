"""
src/train_rfdetr_nano_components.py

Fine-tunes RF-DETR-Nano (Apache-2.0 License) for Model 2 Component Detection.
Classes: plate_char, province (2 classes).
Saves weights to weights/component_detector_rfdetr_nano.pt.
"""
...
TARGET_WEIGHTS_PATH = OUTPUT_WEIGHTS_DIR / "component_detector_rfdetr_nano.pt"
TARGET_ONNX_PATH    = OUTPUT_WEIGHTS_DIR / "component_detector_rfdetr_nano.onnx"
RUNS_DIR            = PROJECT_ROOT / "runs" / "train_rfdetr_nano" / "components"


def train_rfdetr_nano_components(epochs=30, batch_size=4, grad_accum_steps=2, lr=1e-4, patience=10, device=None, export_onnx=False):
    print("=" * 70)
    print("🚀 RF-DETR-Nano Training — Model 2 (Component Detection)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train RF-DETR-Nano for Model 2 (Component Detection)")
