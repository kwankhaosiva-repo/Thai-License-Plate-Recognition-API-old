"""
src/train_grayscale_province_lao.py

Trains Model 3B_Lao Lao Province Classifier using the Grayscale Image Classification approach:
1. Converts all crops to Grayscale to eliminate background color variance (yellow car, blue police, white business).
2. Uses GrayscaleSmartResize((256, 64)) with dynamic background-matching padding.
3. Preserves weights/province_model_lao.pth intact.
4. Saves best model to weights/province_model_grayscale_lao.pth.
"""
...
def train_grayscale_lao(epochs=12, batch_size=32, lr=2e-4):
    ...
    class_to_idx = {v: int(k) for k, v in prov_map.items()}
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"  Validation -> Loss: {val_loss:.4f} | Top-1: {val_top1 * 100:.2f}% | Top-3: {val_top3 * 100:.2f}%")
    ...
    if MODEL_SAVE_PATH.exists():
        ...
        print("\n" + "=" * 70)
        print(f"🏁 Final Lao Test Set Evaluation:")
