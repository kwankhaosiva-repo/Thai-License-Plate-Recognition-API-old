"""
src/train_rtdetr_char_box.py

Fine-tunes RT-DETR-L (Apache-2.0 License) for Model 3A Character Box Detection.
Optimized for Apple Silicon MPS (Metal Performance Shaders).
Exports trained checkpoint to weights/character_box_detector_rtdetr.pt upon completion.
"""
...
DATA_YAML = PROJECT_ROOT / "datasets" / "Thai" / "LPR 2 - Character Box Detection.yolov11" / "data.yaml"
OUTPUT_WEIGHTS_DIR = PROJECT_ROOT / "weights"
TARGET_WEIGHTS_PATH = OUTPUT_WEIGHTS_DIR / "character_box_detector_rtdetr.pt"


def train_rtdetr_char_box():
    print("=" * 70)
    print("🚀 Starting RT-DETR-L Enterprise Training for Model 3A (Character Box Detection)")
    ...
    best_pt = PROJECT_ROOT / "runs" / "train_rtdetr" / "char_box" / "weights" / "best.pt"
    if not best_pt.exists():
        best_pt = PROJECT_ROOT / "runs" / "train_rtdetr" / "char_box" / "weights" / "last.pt"
    ...
    if best_pt.exists():
        ...
        print("⚠️ Warning: Could not find checkpoint weights in runs/train_rtdetr/char_box/weights/")
