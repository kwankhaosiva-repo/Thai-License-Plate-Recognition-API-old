"""
src/train_rfdetr_nano_plate.py

Fine-tunes RF-DETR-Nano (Apache-2.0 License) for Model 1 Car Plate Detection.
Saves weights to weights/plate_detector_rfdetr_nano.pt.
Preserves existing RT-DETR, RF-DETR-Small, and RF-DETR-Base weights.
"""
...
TARGET_WEIGHTS_PATH = OUTPUT_WEIGHTS_DIR / "plate_detector_rfdetr_nano.pt"
TARGET_ONNX_PATH    = OUTPUT_WEIGHTS_DIR / "plate_detector_rfdetr_nano.onnx"
RUNS_DIR            = PROJECT_ROOT / "runs" / "train_rfdetr_nano" / "plate"


def train_rfdetr_nano_plate(epochs=30, batch_size=4, grad_accum_steps=2, lr=1e-4, patience=10, device=None, export_onnx=False):
    print("=" * 70)
    print("🚀 RF-DETR-Nano Training — Model 1 (Plate Detection)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train RF-DETR-Nano for Model 1 (Plate Detection)")
