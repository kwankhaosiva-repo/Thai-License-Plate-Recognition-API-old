"""
src/train_rtdetr_components.py

Fine-tunes RT-DETR-L (Apache-2.0 License) for Model 2 Component Detection (plate_char & province).
Optimized for Apple Silicon MPS (Metal Performance Shaders).
Exports trained checkpoint to weights/component_detector_rtdetr.pt upon completion.
"""
...
DATA_YAML = PROJECT_ROOT / "datasets" / "Thai" / "LPR 2 - Charactor Detection.yolov11" / "data.yaml"
OUTPUT_WEIGHTS_DIR = PROJECT_ROOT / "weights"
TARGET_WEIGHTS_PATH = OUTPUT_WEIGHTS_DIR / "component_detector_rtdetr.pt"


def train_rtdetr():
    print("=" * 70)
    print("🚀 Starting RT-DETR-L Enterprise Training for Model 2 (Components)")
    ...
    # Hyperparameters tuned for Apple Silicon MPS
    train_args = {
        "data": str(DATA_YAML),
        "epochs": 30,
        "patience": 10,
    ...
    # Path to best model
    best_pt = PROJECT_ROOT / "runs" / "train_rtdetr" / "components" / "weights" / "best.pt"
    if not best_pt.exists():
        # Fallback to last.pt if early stopping or direct
        best_pt = PROJECT_ROOT / "runs" / "train_rtdetr" / "components" / "weights" / "last.pt"
    ...
    if best_pt.exists():
        ...
        print("⚠️ Warning: Could not find checkpoint weights in runs/train_rtdetr/components/weights/")
