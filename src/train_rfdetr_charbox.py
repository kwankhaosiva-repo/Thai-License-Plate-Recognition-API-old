"""
src/train_rfdetr_charbox.py

Fine-tunes RF-DETR-Small (Apache-2.0) for Model 3A Character Box Detection.
Same dataset as original RT-DETR: datasets/Thai/LPR 2 - Character Box Detection.yolov11/
Old weights (character_box_detector_rtdetr.pt) are PRESERVED.
"""
...
TARGET_WEIGHTS_PATH = OUTPUT_WEIGHTS_DIR / "character_box_detector_rfdetr.pt"
RUNS_DIR            = PROJECT_ROOT / "runs" / "train_rfdetr" / "charbox"


def train_rfdetr_charbox():
    print("=" * 70)
    print("🚀 RF-DETR-Base Training — Model 3A (Character Box Detection)")
    print(f"   License   : Apache-2.0 ✅")
    print(f"   Classes   : Each Thai Charactor car plate (1 class)")
    print(f"   Dataset   : {DATASET_DIR.name}  (461 images)")
