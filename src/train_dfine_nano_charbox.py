"""
src/train_dfine_nano_charbox.py

Fine-tunes D-FINE Nano (MIT License) for Model 3A Character Box Detection.
Classes: char (1 class).
Saves weights to weights/character_box_detector_dfine_nano.pt and .onnx.
"""
...
TARGET_WEIGHTS_PATH = OUTPUT_WEIGHTS_DIR / "character_box_detector_dfine_nano.pt"
TARGET_ONNX_PATH    = OUTPUT_WEIGHTS_DIR / "character_box_detector_dfine_nano.onnx"
RUNS_DIR            = PROJECT_ROOT / "runs" / "train_dfine_nano" / "charbox"
...
def train_dfine_nano_charbox(epochs=30, batch=8, imgsz=640, patience=12, device="mps", export_onnx=True):
    print("=" * 70)
    print("🚀 D-FINE Nano Training — Model 3A (Character Box Detection)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train D-FINE Nano for Model 3A (Character Box Detection)")
